...
<!-- run-stats:start -->
> **Run date:** 2026-06-25 17:16 UTC — **Severity gate:** `HIGH,CRITICAL`
...
Output files:
- `pipeline-runs.tsv` — TSV with columns `index`, `label`, `run_id`, `severity`
- `pipeline-runs-meta.sh` — run date, severity gate, and image range (read by `pipeline-results.sh`)

### pipeline-results.sh

Reads `pipeline-runs.tsv`, waits for each GitHub Actions run to complete, classifies
the result, prints a summary table, and updates the **Test images** table in `tests.md`.

```bash
# Syntax
./pipeline-results.sh [input.tsv] [tests.md]

# Examples
./pipeline-results.sh                             # reads pipeline-runs.tsv, updates tests.md
./pipeline-results.sh my-run.tsv                  # custom input file
./pipeline-results.sh pipeline-runs.tsv tests.md  # explicit paths
...
- `tests.md` — **Test images** table updated with emoji status + **Pipeline run summary** section updated with statistics
